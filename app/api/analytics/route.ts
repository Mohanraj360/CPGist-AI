...
export async function GET(request: Request) {
  ...
  try {
    ...
    const comparison = brandId && compare ? await brandComparison(supabase, datasetId, [brandId, compare]) : brandId ? await brandComparison(supabase, datasetId, [brandId]) : analytics.brands.slice(0, 8)
    return NextResponse.json({ dataset, ...analytics, selectedBrand, comparison, generatedAt: new Date().toISOString() })
  } catch (error) {
