...
function parseModelResponse(raw: string) {
  ...
  } catch { /* model may have returned plain text */ }
  return raw.trim()
}
...
export async function POST(request: Request) {
  try {
    ...
    let datasetId = typeof body.datasetId === 'string' && /^\d+$/.test(body.datasetId) ? body.datasetId : null
    if (!datasetId) {
      ...
      datasetId = data?.id == null ? null : String(data.id)
    }
    if (!datasetId) return NextResponse.json({ error: 'Select or ingest a dataset before asking the AI Analyst.' }, { status: 422 })

    const { data: dataset, error: datasetError } = await supabase
      .from('datasets').select('id,name,status,row_count,source,updated_at').eq('id', Number(datasetId)).single()
    ...
    // The database is the calculator. The LLM receives already-computed evidence and only explains it.
    const evidenceObject = {
      dataset: { id: dataset.id, name: dataset.name, source: dataset.source, rows: analytics.metrics.rowCount, period: [analytics.metrics.minPeriod, analytics.metrics.maxPeriod] },
      metrics: analytics.metrics,
      trend: analytics.trend,
      categories: analytics.categories,
      brands: analytics.brands,
      retailers: analytics.retailers,
      promotion: analytics.promo,
      elasticity: analytics.elasticity,
      competitiveSignals: analytics.competitiveSignals,
      brandRelationships: analytics.brandRelationships,
      distributionWhitespace: analytics.distributionWhitespace,
      anomalies: analytics.anomalies,
      lineage: analytics.lineage,
    }
    const evidence = JSON.stringify(evidenceObject)
    const raw = await generateWithOllama(`You are CPGist AI, an evidence-first consumer packaged goods analyst. Use ONLY the supplied JSON evidence. Never invent values, brands, periods, sources, causal explanations, or unsupported calculations. Do not recompute metrics; report database-derived values. If the evidence cannot answer the question, say so. Return JSON only: {"answer":"concise answer grounded in evidence"}.\nDATASET EVIDENCE:\n${evidence}\nUSER QUESTION:\n${prompt}`)
    ...
    const result = {
      ...
      trace: [
        'Intent detected',
        'Authorized dataset loaded',
        'Postgres calculated deterministic CPG metrics',
        'Evidence and lineage supplied to model',
        'Dataset evidence verification completed',
      ],
      lineage: analytics.lineage,
    }

    const { data: saved, error: saveError } = await supabase
      .from('analyses').insert({ dataset_id: Number(datasetId), prompt, result, created_by: user.id }).select('id').single()
    if (saveError) console.error('[analyst] save failed', saveError)

    return NextResponse.json({ ...result, analysisId: saved?.id ?? null })
  } catch (error) {
    console.error('[analyst]', error)
    return NextResponse.json({ error: error instanceof Error ? error.message : 'AI Analyst is unavailable.' }, { status: 503 })
  }
}
